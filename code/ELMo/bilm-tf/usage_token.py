...
token representations. 为corpus中所有token建立词汇表，使用model.py中的基于多
kernel CNN拼接的character embedding计算每个token的token embedding并写入到文件中，
然后model中的biLSTM直接使用文件中提前保存的token embedding.
这种方法适用于corpus规模比较小的情况，测试集中不能出现OOV的词，因为是直接使用提前计算的token embedding，
不能处理OOV
