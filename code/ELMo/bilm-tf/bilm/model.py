...
class BidirectionalLanguageModel(object):
    ...
    def _build_ops(self, lm_graph):
        with tf.control_dependencies([lm_graph.update_state_op]):
            ...
            # LM embeddings包含三层embedding(char-embed + 2-concat(forward-lstm, backward-lstm))
            # 所以LM embeddings shape: [batch_size, 3, seq_len, embedding_size]
            token_embeddings = lm_graph.embedding
            # token embedding没有前后向之分，所以复制一份进行拼接
            layers = [
            ...
            # lm_graph.sequence_lengths是batch中每个句子的长度int,shape:[batch_size,],eg.[8,5,4,7]
...
# 模型架构图：多kernel size的CNN char-embed拼接+2 highway network+2-bilstm
class BidirectionalLanguageModelGraph(object):
    ...
    def _build_word_char_embeddings(self):
        ...
        # ascii(256) + 词首词尾 + 句首句尾 + char-pad + sent-pad, 参考data.UnicodeCharsVocabulary
        if n_chars != 262: 
        ...
        # the convolutions，使用多跨度即多kernel size的CNN char-embed，将每个CNN kernel
        # 处理得到的embedding进行拼接
...
def dump_bilm_embeddings(vocab_file, dataset_file, options_file,
    ...
    with tf.Session(config=config) as sess:
        ...
        with open(dataset_file, 'r') as fin, h5py.File(outfile, 'w') as fout:
            for line in fin:
                # 单句处理保证返回的lm_embedding[1,3,seq_len,embedding_size]中seq_len为句长，
                # 因为没有多句对齐的padding，同时lm_embedding已经去除句首句尾标记
