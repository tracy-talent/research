...
# tokenizer_pt = tfds.features.text.SubwordTextEncoder.build_fr496om_corpus(
...
# 指定输入大小，避免在graph mode下为每个不同shape的输入cache graph
...
# Evaluate
def beam_search(source_sentence):   # 已编码的词id组成的句子
  ...
  candidates_translation = []
  beam_logprob = np.zeros(beam_size, dtype=np.float32)
  beam_next_ids = np.zeros(beam_size * beam_size, dtype=np.int32)
  beam_next_logprob = np.zeros(beam_size * beam_size, dtype=np.float32)
  for i in range(MAX_LENGTH):
    beam_next_logprob[:] = -np.inf
    for beam_step in range(seap):
      ...
      beam_next_ids[beam_step * beam_size : (beam_step + 1) * beam_size] = np.argpartition(predictions[: ,-1:, :], -beam_size, axis=-1)[0, -1, :-(beam_size+1):-1]  # (batch_size, 1, vocab_size)
          ...
          if i > 0:
            ...
            if transidx != []:
              ...
              if (len(candidates_translation) == candidates_num): 
                break
          beam_next_logprob[beam_step * beam_size + j] = -np.inf
  ...
  if candidates_score != []:
    print(candidates_translation[np.argmax(candidates_score)][1])
